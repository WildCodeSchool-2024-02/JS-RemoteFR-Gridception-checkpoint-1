...
function findAdults(persons) {
  ...
  persons.forEach((person) => {
    if ((persons) => person.sex === "female" && person.age >= 18);
    if ((persons) => person.sex === "male" && person.age >= 18);

    return [femaleAdults, maleAdults];
  });
}
console.log(findAdults(femaleAdults));
console.log(findAdults(maleAdults));
